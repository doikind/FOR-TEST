...
全新单体应用（此前 FastAPI 原型已弃用，不在交付范围）。约束见 proposal.md：两天交付、Python + Streamlit + SQLite、两题合并闭环、三类真实公开数据源（Google News RSS + GDELT + Hacker News，CoinGecko 仅可选补充）、数据真实性三标签、无 API Key 可完整运行、所有账号操作人工确认、不接真实 X 发布、不购买 X API、不引入 LangChain/CrewAI 等重型框架。技术选型追求"少依赖、低风险、快演示"，用普通 Python 函数 + 清晰工作流实现。
...
- **理由**：两天内交付 UI 成本最低，无需前后端分离；官方支持 `st.session_state` 管理交互态；演示时"人工确认点"可视化直观。
...
- **备选**：JSON 文件（原型用过，并发与查询弱）；Postgres（过重，违背两天约束）。
...
- **理由**：满足题目硬性要求"模拟数据必须明确标识"；快照兜底是演示离线保障，必须与实时数据可区分，禁止静默伪装。
...
- **理由**：题目点名要求 scikit-learn 的 TF-IDF 与余弦相似度；小规模语料（每日 ≤ 100 事件、候选 ≤ 50 条）O(n²) 可接受，零额外基础设施。
- **备选**：纯字符串 SequenceMatcher（精度差，且不满足题目对 sklearn 的要求）；datasketch MinHash-LSH（万级才值得，依赖更重）。
...
- **理由**：确定性规则可复现、可解释（评审看重"判断逻辑清晰"），且无 Key 也可运行。
...
- **备选**：业务代码直接调 SDK（耦合、无 Key 即瘫痪）；LangChain/CrewAI（题目明确不引入）。
...
- **理由**：两天内可实现、可解释、合规（不抓私密数据、不声称拥有非公开指标）；避免黑箱 ML。
...
- [模拟表现被误认为真实成绩] → 全部 `simulated_demo` 徽标 + README/演示脚本强调。
- [两天工期紧] → 严格按 tasks.md 分层推进：先跑通 P0 最小闭环，再增强（拆解、二次创作、复盘、快照）。
...
（无阻塞性未知项。以下留待实现期确认，不影响 spec 与任务拆分：Finimize 与 AlphaSense 二者公开数据可得性在 Day 2 上午实现时实地验证并择一为主案例；OpenAI 兼容端点默认指向 OpenAI 还是 DeepSeek——由 `.env` 的 `OPENAI_BASE_URL` 决定，两者皆可。）
